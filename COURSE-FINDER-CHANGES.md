## Continuation — 14 September 2026, build 16 (back tees by default)

- Every OpenStreetMap-built map now starts each hole from the **furthest-back mapped tee box** (the championship tee) until a scorecard picks a tee set. The assembler gathers the `golf=tee` boxes in a corridor around the start of each hole's routing (80 m behind to 120 m ahead, 30 m either side, and not nearer to another hole's start), keeps them all on the hole as `tees` (name and playing length each), and measures the hole from the one furthest from the green. Bare hole numbers on tee boxes are not treated as tee names. Augusta National: 18/18 holes from the back tees, 7,479 yd (Masters card 7,555 yd). The existing card logic then shortens the hole forward along the routing to match the card's yardage when one is applied.
- The main map labels the tee the hole is measured from (yellow pill, with the number of tee boxes on the hole). Maps downloaded by earlier builds are re-downloaded once at start-up (2–3 s, online only) so they also start from the back tees; played holes keep their shots.
- Verified: 24 unit regressions, the mobile browser regression, live Augusta download, and a simulated older saved map being refreshed at boot. Not verified on a physical iPhone in this run.
