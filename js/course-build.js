...
export function buildCourseData(name, location, els, card, opts = {}) {
  ...
  // Eighteen unique, consecutive hole lines are an 18-hole layout even when the course outline
  // carries no holes=* tag (Augusta, most named layouts). Nine alone are not proof: it may be
  // the mapped half of an 18-hole course.
  const eighteen = refs.length === 18 && [...refs].sort((a, b) => a - b).every((n, i) => n === i + 1);
  const expectedHoles = opts.expectedHoles ?? (card?.holes?.length === 18 || eighteen ? 18 : null);
